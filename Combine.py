...
def authenticate_user_with_rnn(audio_file, model_path):
    ...
    input_features = input_features.reshape((1, input_features.shape[0], 1))  # (1, 시간 단계, 특징 수)
...
def register_user(username):
    # 사용자 음성 파일 경로
    audio_file = f"C:\\voice\\wav\\{username}_register.flac"  # 사용자 음성 파일 경로
    ...
    features = extract_mfcc_features_from_flac(audio_file)
    features = features.reshape((1, features.shape[0], 1))  # (1, 특징 수, 1)

    model = create_rnn_model(input_shape=(features.shape[1], 1))
    model.fit(features, np.array([1]))  # 예시로 해당 사용자의 라벨은 1 (등록됨)

    # 모델 저장
    model.save(f"C:\\voice\\models\\{username}\\voice_authentication_model.h5")
